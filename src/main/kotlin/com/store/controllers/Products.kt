...
class Products {
    ...
    fun createProduct(@RequestBody productDetails: ProductDetails): ResponseEntity<Any> {
        return if (productDetails.name.isEmpty() || productDetails.inventory <= 0 ||
            productDetails.name.contains("[0-9]".toRegex()) || listOf(
                "true",
                "false"
            ).contains(productDetails.name) || !listOf(
                "gadget",
                "book",
                "food",
                "other"
            ).contains(productDetails.type) || productDetails.cost <= 0.0
        ) {
            ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
                ErrorResponseBody(
                    timestamp = LocalDateTime.now().format(DateTimeFormatter.ISO_DATE_TIME),
            ...
            ResponseEntity.status(HttpStatus.CREATED).body(product.id)
        }
    }
}